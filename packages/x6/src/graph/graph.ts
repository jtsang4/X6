...
export class Graph extends Basecoat<EventArgs> {
  ...
  constructor(options: Partial<GraphOptions.Manual>) {
    ...
    this.css = new CSSManager(this) // 添加 X6 的样式
    this.hook = new HookManager(this) // 各种模块的管理和创建类，可以通过 options 选项传入对应方法来修改默认值
    this.view = this.hook.createView() // 创建 graph 的 view，除创建 DOM 元素外，还委托了事件到 container 上
    this.defs = this.hook.createDefsManager() // defs 元素的管理器
    this.coord = this.hook.createCoordManager() // 坐标定位和转换
    this.transform = this.hook.createTransformManager() // 节点伸缩、旋转和平移
    this.knob = this.hook.createKnobManager() // 节点的旋钮
    this.highlight = this.hook.createHighlightManager() // cell 的高亮
    this.grid = this.hook.createGridManager() // 背景网格
    this.background = this.hook.createBackgroundManager() // 背景大小
    this.model = this.hook.createModel() // 创建 graph 的 model，并在 model 内绑定 graph，内部有 collection model 来管理 graph cells 的 model
    this.renderer = this.hook.createRenderer() // graph 渲染层的控制模块，在 model 变化时把变动映射到 view 上
    this.clipboard = this.hook.createClipboardManager() // 处理 cell 的复制、剪切和粘贴
    this.snapline = this.hook.createSnaplineManager() // 对齐线管理器
    this.selection = this.hook.createSelectionManager() // 处理 cell 选中的管理器
    this.history = this.hook.createHistoryManager() // 实现 undo, redo 相关的操作
    this.scroller = this.hook.createScrollerManager() // 实现画布的平移
    this.minimap = this.hook.createMiniMapManager() // 实现 minimap
    this.keyboard = this.hook.createKeyboard() // 实现画布上的键盘事件绑定
    this.mousewheel = this.hook.createMouseWheel() // 处理画布滚动
    this.print = this.hook.createPrintManager() // 处理打印相关
    this.format = this.hook.createFormatManager() // 导出相关
